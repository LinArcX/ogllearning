...
void GLSLProgram::compile_shaders(const std::string& vertex_shader_file_path, const std::string& fragment_shader_file_path)
{
	...
	{
		ogl::LoggerManagerProvider::getLoggerManager()->fatal("Can't create vertext_shader!");
	}

	m_fragment_shader_id = glCreateShader(GL_FRAGMENT_SHADER);
	if (!m_fragment_shader_id)
	{
		ogl::LoggerManagerProvider::getLoggerManager()->fatal("Can't create fragment_shader!");
	}
...
GLint GLSLProgram::get_uniform_location(const std::string& uniform_name)
{
	...
	{
		ogl::LoggerManagerProvider::getLoggerManager()->fatal("Uniform: " + uniform_name + ", not found in shader!");
	}
	return location;
}
...
void GLSLProgram::compile_shader(const std::string& shader_file_path, GLuint shader_id)
{
	...
	if (shader_file.fail())
	{
		ogl::LoggerManagerProvider::getLoggerManager()->fatal("Failed to open: " + shader_file_path);
	}
	...
	if (GL_FALSE == success)
	{
		...
		ogl::LoggerManagerProvider::getLoggerManager()->fatal("Failed to compile: "
				+ shader_file_path
				+  "!\n"
				+ &(error_log[0]));
	}
...
void GLSLProgram::link_shaders()
{
	...
	if (isLinked == GL_FALSE)
	{
		...
		// +  &(error_log[0])
		ogl::LoggerManagerProvider::getLoggerManager()->fatal("Failed to link: !\n" );
	}
